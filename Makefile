...
BINS := $(addprefix bin/$(ARCH)/,kove-extended-resource-controller)
PROJECT := kove-extended-resource-controller
...
BUILD_IMAGE ?= golang:1.17.7-alpine
...
tmp/help.txt: bin/$(ARCH)/kove-extended-resource-controller
	mkdir -p tmp
	bin/$(ARCH)/kove-extended-resource-controller --help 2>&1 | head -n -1 > $@
